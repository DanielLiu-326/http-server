//
// Created by Danny on 2020/8/6.
//

#ifndef TCPPKGLY_FIFOBUFFER_H
#define TCPPKGLY_FIFOBUFFER_H
#include<mutex>
#include<queue>
#include<unistd.h>
...
public:
    ...
    inline bool empty();
    static MemoryPool<FifoBlock<blockSize> > blockPool;
...
template<int blockSize>
std::mutex FifoBuffer<blockSize>::poolLock;

template<int blockSize>
MemoryPool<FifoBlock<blockSize> > FifoBuffer<blockSize>:: blockPool(50,1);
...
int FifoBuffer<blockSize>::writeOut(int fd, int maxSize)//return < 0 error return >=0 write finished or eagain
{
    ...
    {
        int len = ::write(fd,buffer.front()->dataNow(),MIN(buffer.front()->surplus(),maxSize-now));
        if(len<0)
        {
            if(errno==EAGAIN)
            {
...
int FifoBuffer<blockSize>::writeIn(const char *data, int length)
{
    ...
    for(now = 0;now < length;)
    {
        ...
        {
            this->poolLock.lock();
            this->buffer.pushBack(new(this->blockPool.alloc()) FifoBlock<blockSize>);
            this->poolLock.unlock();
        }
        else
        {
